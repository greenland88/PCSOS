...
def build_market_context(symbol: str, as_of: date | str, *, data_access: PCSDataAccess,
                         ...
                         benchmark_frame: pd.DataFrame | None = None) -> MarketContext:
    ...
    benchmark = (benchmark_frame.copy() if benchmark_frame is not None else
                 data_access.read_prices("QQQ", end_date=cutoff)).sort_values("date").drop_duplicates("date", keep="last").reset_index(drop=True)
    ...
    def _above(ticker, period):
        frame = data_access.read_prices(ticker, end_date=cutoff).sort_values("date")
        close = pd.to_numeric(frame.close); return bool(close.iloc[-1] > close.rolling(period, min_periods=period).mean().iloc[-1])
    spy = _above("SPY", 50); qqq50 = _above("QQQ", 50)
    qqq = data_access.read_prices("QQQ", end_date=cutoff).sort_values("date"); qclose = pd.to_numeric(qqq.close)
    ...
    state = MarketState(qqq_above_20dma=qqq20, qqq_above_50dma=qqq50, qqq_above_200dma=qqq200,
                        spy_above_50dma=spy, soxx_above_50dma=_above("SOXX", 50), breadth_positive=spy and qqq50)
