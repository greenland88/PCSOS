...
from pcs.trend.selection_models import (
    ...
    OpportunityEpisode, OpportunityStateCheckpoint,
)
...
def _structure(bar, details):
    detail = details.get(bar.session.isoformat())
    if detail and detail.high_comparison == detail.low_comparison == 'lower':
        return 'bearish'
    return detail.structure_state if detail and detail.structure_state is not None else bar.structure_state


def _formation(input, session, sessions, bars, details, policy_hash, source_hash):
    ...
    uptrend = [s for s in preceding if s in bars and _structure(bars[s], details) == 'bullish']
    unknown_structure = [s for s in preceding if s not in bars or _structure(bars[s], details) is None]
    ...
    state = _structure(bars[session], details)
    ...
    return BaseFormationCandidate(candidate_id=candidate_id, session=session,
...
def detect_constructive_base(input: ConstructiveBaseInput) -> BaseResult:
    ...
    for session in process:
        ...
        if event and event.state not in TERMINAL:
            ...
            structure = _structure(bar, details)
            ...
            if required_conjunction(conditions) is False:
                ...
                if session > event.confirmation_deadline:
                    ...
                    confirm = confirmation_conditions(bar=bar, previous_bar=bars.get(sessions[sessions.index(session)-1]),
                        history=list(bars.values()), zone_upper=zone.upper, anchor_atr=zone.anchor_atr,
            ...
            zone, _ = update_fixed_support_zone(event.lower_zone, _support_bar(bar), sessions, input.support_policy, [])
            event = event.model_copy(update={'lower_zone': zone,
                'base_validity': 'INVALIDATED' if zone.broken_at or _structure(bar, details) == 'bearish' else
                    'UNKNOWN' if _structure(bar, details) is None else event.base_validity})
            events[-1] = event
        ...
        if may_form:
            candidate = _formation(input, session, sessions, bars, details, policy_hash, source_hash)
        ...
        timeline.append(BaseDay(session=session, **_snapshot(event), base_detected=detected,
...
def base_explanation(event, last, requested, eligible):
    if event is None:
        return f'尚未形成平台；当前必要证据{"未知" if eligible is None else "未全部通过"}。逐日候选条件解释未成立原因；形态不代表公司质量或下单授权。'
    ...
    return (f'{event.formed_at}形成固定平台，上沿{event.upper:g}、下沿{event.lower:g}；历史守住：{held}，'
        ...
        f'请求交易日{requested}，资格{"未知" if eligible is None else "可评估" if eligible else "不可入场"}；不是下单授权。')
