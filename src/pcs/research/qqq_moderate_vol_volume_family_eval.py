...
def run():
    ...
    family["episode_id"]=(family.trade_date.diff().dt.days.fillna(999)>4).cumsum(); e=family.groupby("episode_id",as_index=False).first()
    ...
    (ART/"moderate_vol_volume_family_eval.json").write_text(json.dumps(out,indent=2,default=str),encoding="utf-8"); print(json.dumps(out,indent=2,default=str)); return out
