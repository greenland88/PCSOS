...
def run():
    ...
    h=d[d.vol_pct_rank.between(.429,.753,inclusive="right")].copy(); h["episode_id"]=(h.trade_date.diff().dt.days.fillna(999)>4).cumsum(); e=h.groupby("episode_id",as_index=False).first()
    ...
    (ART/"h002_volatility_audit.json").write_text(json.dumps(out,indent=2,default=str),encoding="utf-8"); print(json.dumps(out,indent=2,default=str)); return out
