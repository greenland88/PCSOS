...
def run():
 d=pd.read_parquet(ART/"qqq_state_transition_features_train_2020_2023.parquet"); d.trade_date=pd.to_datetime(d.trade_date).dt.normalize(); f=d[(d.close_sma200_atr<0)&(d.ret10>0)&(d.ret5>0)].copy(); f["episode_id"]=(f.trade_date.diff().dt.days.fillna(999)>4).cumsum(); e=f.groupby('episode_id',as_index=False).first()
 ...
 (ART/"h008_breakdown_recovery.json").write_text(json.dumps(out,indent=2,default=str),encoding='utf-8'); print(json.dumps(out,indent=2,default=str)); return out
