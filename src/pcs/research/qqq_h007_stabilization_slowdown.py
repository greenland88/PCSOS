...
def run():
 ...
 f["episode_id"]=f.session_index.diff().fillna(999).ne(1).cumsum(); groups={i:g.sort_values('trade_date') for i,g in f.groupby('episode_id')}; first=[]; h006=[]; h007=[]
 ...
 target=ART/"h007_stabilization_slowdown.json"; temp=ART/f".{target.name}.{uuid.uuid4().hex}.tmp"
