...
def run():
 d=pd.read_parquet(ART/"qqq_state_transition_features_train_2020_2023.parquet"); d.trade_date=pd.to_datetime(d.trade_date).dt.normalize(); f=d[(d.drawdown60<=-.02)&(d.ret10>0)].copy(); f['episode_id']=(f.trade_date.diff().dt.days.fillna(999)>4).cumsum(); base=[]; h006=[]; h013=[]
 ...
 (ART/"h013_stabilization_one_day_delay.json").write_text(json.dumps(out,indent=2,default=str),encoding='utf-8'); print(json.dumps(out,indent=2,default=str)); return out
