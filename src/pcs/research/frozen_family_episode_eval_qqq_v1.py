...
def main():
 ...
 sessions=pd.DatetimeIndex(d.trade_date.dt.normalize().drop_duplicates().sort_values()); positions=pd.Series(range(len(sessions)),index=sessions)
 ...
 for hid,(family,mask) in specs.items():
  g=d[mask].copy(); g['session_index']=g.trade_date.map(positions); g['episode_id']=(g.session_index.diff().fillna(999).ne(1)).cumsum(); first=g.groupby('episode_id',as_index=False).first(); out['families'][hid]={'setup_family':family,'frozen_rule':'single broad observed tercile; one entry at first date per episode','all_qualifying':metric(g),'one_entry_per_episode':metric(first),'year_metrics':{str(y):metric(x) for y,x in first.assign(year=first.trade_date.dt.year).groupby('year')}}
 target=ART/'frozen_family_episode_eval.json'; temp=target.with_name(f'.{target.name}.{os.getpid()}.{uuid.uuid4().hex}.tmp'); temp.write_text(json.dumps(out,indent=2,default=str)); os.replace(temp,target); print(json.dumps(out,indent=2,default=str))
