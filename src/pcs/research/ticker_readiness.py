...
def _daily_checks(r, daily, expected_dates=None, executable_start_date=None):
...
def preflight_ticker(symbol: str, *, access=None, run_id=None, request_id=None) -> TickerReadiness:
    ...
    try:
        ...
        qa=access.audit_options_quality(s, start_date=executable_start); dup=int(qa["duplicate_option_rows"]); conflicts=int(qa["ambiguous_conflicting_option_keys"]); usable=int(qa["usable_30_45_dte_rows"]); valid_quotes=int(qa["valid_30_45_dte_quote_rows"]); valid_exp_rows=int(qa["valid_30_45_dte_expiration_rows"]); valid_strike_rows=int(qa["valid_30_45_dte_strike_rows"]); valid_bid=valid_quotes == usable; valid_exp=valid_exp_rows == usable; valid_strike=valid_strike_rows == usable; r.checks["options"]={"route":ev,"executable_start_date":executable_start,"duplicate_option_rows":dup,"duplicate_option_keys":int(qa["duplicate_option_keys"]),"identical_duplicate_keys":int(qa["identical_duplicate_keys"]),"ambiguous_conflicting_option_keys":conflicts,"valid_bid_ask":valid_bid,"valid_expirations":valid_exp,"valid_strikes":valid_strike,"usable_30_45_dte_rows":usable,"valid_30_45_dte_quote_rows":valid_quotes,"invalid_30_45_dte_quote_rows":usable-valid_quotes}
        ...
        detail = str(e)
        code = "OPTIONS_DATA_NOT_INGESTED" if "DATA_NOT_INGESTED" in detail else "OPTIONS_ROUTE_OR_SOURCE_UNAVAILABLE"
        _block(r, "OPTIONS", code, detail)
