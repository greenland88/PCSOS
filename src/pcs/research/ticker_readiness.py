...
def preflight_ticker(symbol: str, *, access=None, run_id=None, request_id=None) -> TickerReadiness:
    ...
    try:
        ...
        qa=access.audit_options_quality(s, start_date=executable_start); dup=int(qa["duplicate_option_rows"]); conflicts=int(qa["ambiguous_conflicting_option_keys"]); usable=int(qa["usable_30_45_dte_rows"]); valid_quotes=int(qa["valid_30_45_dte_quote_rows"]); valid_exp_rows=int(qa["valid_30_45_dte_expiration_rows"]); valid_strike_rows=int(qa["valid_30_45_dte_strike_rows"]); null_identity=int(qa.get("null_identity_rows", 0)); valid_bid=valid_quotes == usable; valid_exp=valid_exp_rows == usable; valid_strike=valid_strike_rows == usable; r.checks["options"]={"route":ev,"executable_start_date":executable_start,"duplicate_option_rows":dup,"duplicate_option_keys":int(qa["duplicate_option_keys"]),"identical_duplicate_keys":int(qa["identical_duplicate_keys"]),"ambiguous_conflicting_option_keys":conflicts,"null_identity_rows":null_identity,"valid_bid_ask":valid_bid,"valid_expirations":valid_exp,"valid_strikes":valid_strike,"usable_30_45_dte_rows":usable,"valid_30_45_dte_quote_rows":valid_quotes,"invalid_30_45_dte_quote_rows":usable-valid_quotes}
