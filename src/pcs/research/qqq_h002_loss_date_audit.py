...
def run():
    d=pd.read_parquet(ART/"qqq_state_transition_features_train_2020_2023.parquet"); d.trade_date=pd.to_datetime(d.trade_date).dt.normalize(); h=d[d.vol_pct_rank.between(.429,.753,inclusive="right")].copy(); h["episode_id"]=(h.trade_date.diff().dt.days.fillna(999)>4).cumsum(); e=h.groupby("episode_id",as_index=False).first(); losses=e[e.outcome_class!="GOOD_WIN"]; wins=e[e.outcome_class=="GOOD_WIN"]
    def rows(x):
    ...
    (ART/"h002_loss_date_audit.json").write_text(json.dumps(out,indent=2,default=str),encoding="utf-8"); print(json.dumps(out,indent=2,default=str)); return out
