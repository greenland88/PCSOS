...
def run_transfer(request: TransferRequest, *, data_access: PCSDataAccess | None = None, output_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    result = {"module":"pcs.research.strategy_transfer_runner","version":"v1","status":"COMPLETED_DESCRIPTIVE","strategy_id":request.strategy_id,"ticker":ticker,"train_start":request.train_start,"train_end":request.train_end,"data_dependencies":sorted(spec.data_dependencies),"qualifying_dates":[d.date().isoformat() for d in qualifying],"independent_episodes":episodes,"executable_episodes":[],"trades":[],"performance":{},"reason_codes":["EXACT_TRANSFER","PIT_FEATURES","CANONICAL_DATA_VALIDATED","DECLARED_DATA_DEPENDENCIES","CONTRACT_LIFECYCLE_DELEGATED"]}
    if output_dir:
