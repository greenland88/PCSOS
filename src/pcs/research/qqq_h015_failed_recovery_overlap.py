...
def run():
 d=pd.read_parquet(ART/"qqq_state_transition_features_train_2020_2023.parquet"); d.trade_date=pd.to_datetime(d.trade_date).dt.normalize(); f=d[(d.drawdown60<=-.02)&(d.ret10>0)].copy(); f['episode_id']=(f.trade_date.diff().dt.days.fillna(999)>4).cumsum(); h006=[]; h014=[]; both=[]; only006=[]; only014=[]; neither=[]
 ...
 (ART/"h015_failed_recovery_overlap.json").write_text(json.dumps(out,indent=2,default=str),encoding='utf-8'); print(json.dumps(out,indent=2,default=str)); return out
