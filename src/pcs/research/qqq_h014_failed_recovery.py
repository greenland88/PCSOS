"""Descriptive QQQ H014: failed recovery after a controlled reset."""
...
def run():
 d=pd.read_parquet(ART/"qqq_state_transition_features_train_2020_2023.parquet"); d.trade_date=pd.to_datetime(d.trade_date).dt.normalize(); f=d[(d.drawdown60<=-.02)&(d.ret10>0)&(d.ret5<=0)].copy(); f['episode_id']=(f.trade_date.diff().dt.days.fillna(999)>4).cumsum(); e=f.groupby('episode_id',as_index=False).first(); out={"module":"pcs.research.qqq_h014_failed_recovery","version":"v1","status":"DESCRIPTIVE_ONLY","data_source":"PCS_CANONICAL_DATA","research_mode":"EXISTING_TRADE","hypothesis_id":"QQQ_V1_H014","logic":"QQQ has a positive 10-day response from a drawdown but no positive 5-day response, representing failed or incomplete recovery.","qualifying_dates":int(len(f)),"independent_episodes":int(len(e)),"one_entry_per_episode":metric(e),"year_metrics":{str(y):metric(g) for y,g in e.groupby(e.trade_date.dt.year)},"threshold_mining":False,"new_options_replay":False,"validation_read":False,"final_oos_read":False,"production_changes":False,"reason_codes":["PIT_SAFE_FEATURES","ONE_ENTRY_PER_EPISODE","STRUCTURAL_PATH_HYPOTHESIS","DESCRIPTIVE_ONLY"]}; (ART/"h014_failed_recovery.json").write_text(json.dumps(out,indent=2,default=str),encoding='utf-8'); print(json.dumps(out,indent=2,default=str)); return out
