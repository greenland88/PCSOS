...
def canonical_snapshot_descriptor(*, dataset: str, symbol: str, frame: pd.DataFrame,
    ...
    if "date" not in frame.columns: raise DataQualityError("DATE_COLUMN_MISSING")
    dates=pd.to_datetime(frame["date"], errors="coerce")
    if dates.isna().any() or not dates.is_unique or not dates.is_monotonic_increasing: raise DataQualityError("DAILY_DATE_ORDER_INVALID")
