...
def main():
    OUT.mkdir(exist_ok=True); access=PCSDataAccess(); all_rows=[]; yearly=[]; manifest=[]
    for ticker in TICKERS:
        ...
        # Load the complete canonical prefix once; do not use a provider
        # cache keyed by the first as-of date, which can truncate later rows.
        daily=access.read_prices(ticker, None, tr.exit_date.max()); daily.date=pd.to_datetime(daily.date)
    ...
    (OUT/'run_metadata.json').write_text(json.dumps({'module':PRODUCER,'version':'1.0','status':'RESEARCH_ONLY','calculation_version':CALCULATION_VERSION,'reason_codes':['RESEARCH_ONLY','NO_PRODUCTION_BLOCK'],'source':'PCSDataAccess canonical daily + frozen variant_b_full populations','tickers':manifest},indent=2))
