"""Round 51: NVDA volatility and participation expansion transitions."""
...
def run():
 d=pd.read_parquet(OUT/"pit_feature_outcome_table.parquet").copy(); d.trade_date=pd.to_datetime(d.trade_date).dt.normalize(); d.date=pd.to_datetime(d.date).dt.normalize(); d=d[(d.ticker=="NVDA")&d.trade_date.between("2020-01-02","2023-12-31")].sort_values("trade_date").copy(); atr=d.nvda_atr14.median(); d["prior_atr"]=d.nvda_atr14.shift(1); d["prior_volume"]=d.nvda_volume_rel20.shift(1); trend=(d.nvda_close_vs_sma200>0)&(d.nvda_volume_rel20>1)&(d.nvda_ret5>0); recovery=(d.nvda_close_vs_sma200>0)&(d.nvda_ret20<0)&(d.nvda_ret5>0); modes={"QUIET_TO_VOLATILITY_EXPANSION":(d.prior_atr<=atr)&(d.nvda_atr14>atr)&(d.nvda_ret5>0),"LOW_TO_HIGH_PARTICIPATION_EXPANSION":(d.prior_volume<1)&(d.nvda_volume_rel20>=1)&(d.nvda_ret5>0),"JOINT_VOLATILITY_PARTICIPATION_EXPANSION":(d.prior_atr<=atr)&(d.nvda_atr14>atr)&(d.prior_volume<1)&(d.nvda_volume_rel20>=1)&(d.nvda_ret5>0)}; rows=[]; yearly=[]
