...
quotes=access.read_quotes('NVDA',px.date.min(),pd.Timestamp('2023-12-31')+pd.Timedelta(days=50)); quotes.trade_date=pd.to_datetime(quotes.trade_date).dt.normalize(); quotes.expiration_date=pd.to_datetime(quotes.expiration_date).dt.normalize(); by_day={d:g for d,g in quotes.groupby('trade_date')}; rules=load_rules(); rules['entry']['hard_dte_min']=30; rules['entry']['hard_dte_max']=45; rules['entry']['safe_strike_atr']=2.3; rules['entry']['min_credit_width_ratio']=.10; cal=_load_replay_calendar('data/raw/events/official_event_dates_2010-01-01_to_2026-07-31.csv'); pb=load_corporate_actions(ROOT/'config/data/corporate_actions.csv')
rows=[]
