...
for eid,g in d.groupby('episode_id'):
 s,e=g.date.min(),g.date.max(); start_idx=nvda_sessions.get_loc(s); warm_start=nvda_sessions[max(0,start_idx-5)]; z=p.loc[warm_start:e].reset_index(); z['episode_id']=eid; tl.append(z); row={'episode_id':eid,'episode_start':s,'episode_end':e,'first_baseline_entry':s}
