...
def episodes(x,gap,sessions):
 if x.empty:return pd.DataFrame()
 pos={d:i for i,d in enumerate(sessions)}; groups=[]; current=[]; start_i=None
 for _,r in x.iterrows():
  if r.entry not in pos: raise ValueError(f"MISSING_CANONICAL_SESSION:{r.entry.date()}")
  i=pos[r.entry]
 ...
 for n,g in enumerate(groups,1):
  z=pd.DataFrame(g); end=z.exit.max() if z.exit.notna().any() else z.entry.max(); days=sessions[(sessions >= z.entry.min()) & (sessions <= end)]
...
def main():
 ...
 for t in ('SPY','QQQ'):
  sessions=pd.DatetimeIndex(pd.to_datetime(access.read_prices(t)["date"]).dt.normalize().drop_duplicates().sort_values())
  for split in ('TRAIN','VALIDATION'):
   base=source(t,split); cycles=exposure_cycles(base)
   for gap in (10,15,20):
    ep=episodes(base,gap,sessions); ep['ticker']=t; ep.to_csv(OUT/f'{t.lower()}_{split.lower()}_episodes_{gap}d.csv',index=False); summaries.append({'ticker':t,'split':split,'baseline_trades':len(base),'exposure_cycles':cycles,'opportunity_episodes':len(ep),'trades_per_episode':len(base)/len(ep) if len(ep) else None,'episode_distribution':ep.number_of_entries.value_counts().to_dict()})
    for v in ('R1_MAX_OPEN_1','R2_MAX_OPEN_2'):
     y=source(t,split,v); ve=episodes(y,gap,sessions); retained.append({'ticker':t,'split':split,'variant':v,'gap_days':gap,'baseline_episodes':len(ep),'variant_episodes':len(ve),'pct_opportunity_episodes_retained':len(ve)/len(ep)*100 if len(ep) else None})
