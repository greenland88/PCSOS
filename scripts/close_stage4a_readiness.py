...
def build_nvda():
    ...
    out=pd.DataFrame(rows); _atomic_parquet(out, ROOT/"candidate_inputs"/"NVDA.parquet"); return out
...
def event_audit():
    ...
    out=pd.DataFrame(rows); _atomic_parquet(out,ROOT/"stage4a_event_readiness_ex_post_historical.parquet"); return out
...
def main():
    ...
    _atomic_json(result, ROOT/"final_readiness_audit.json"); print(json.dumps(result,indent=2,default=str))
