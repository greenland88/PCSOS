...
import pandas as pd
from pcs.research.credit_stop import load_quotes
...
def main():
    ...
    for ticker,path in PATHS.items():
        ...
        root=Path("data/parquet/options_v2/rebuild_20260820")
        files=list((root/f"symbol={ticker}").rglob("*.parquet"))
        if not files and ticker=="AMD": files=list(Path("data/parquet/options_v2_onboarding_amd_20260820").rglob("*.parquet"))
        ...
        meta={"files":len(files),"option_rows_loaded":len(chain),"source":str(root)}
        rows=[]; invalid=0
