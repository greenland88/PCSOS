...
def main():
    ...
    for ticker,path in PATHS.items():
        ...
        resolved=access.resolve_source("options", ticker, start, end)
        ...
        meta={"option_rows_loaded":len(chain),"source":"PCSDataAccess.resolve_source(options)","resolved_dataset":resolved.dataset,
        ...
        for day,group in chain.groupby("Trade Date"):
            for row in generate_structural_put_opportunities(group,ticker,day):
                ...
                row["pit_asof"]=str(day.date()); row["source_provenance"]=f"PCSDataAccess:{resolved.dataset}:{ticker}"; rows.append(row)
    ...
    parquet_target=OUT/"production_opportunity_universe.parquet"
    parquet_tmp=OUT/f".{parquet_target.name}.{uuid.uuid4().hex}.tmp"
    try:
        out.to_parquet(parquet_tmp,index=False); os.replace(parquet_tmp,parquet_target)
    finally:
        parquet_tmp.unlink(missing_ok=True)
    json_target=OUT/"production_contract_validation.json"
    json_tmp=OUT/f".{json_target.name}.{uuid.uuid4().hex}.tmp"
    try:
        json_tmp.write_text(json.dumps(report,indent=2,default=str),encoding="utf-8"); os.replace(json_tmp,json_target)
    finally:
