...
def run(split, start, end):
 ...
 for t in ('SPY','QQQ'):
  ...
  for r in d.itertuples():
   day=pd.Timestamp(r.date).normalize(); q,meta=load_quotes_duckdb(':memory:',t,day,day); context=providers[t]({'date':day,'candidate_id':f'{t}-{day.date()}'})
   if day not in mkt or not context.get('available') or context.get('snapshot') is None:
    rows.append({'split':split,'ticker':t,'date':day,'status':'UNKNOWN','reason':'MARKET_OR_TREND_CONTEXT_UNAVAILABLE','selected':False,'option_rows':meta['option_rows_loaded']});continue
   chosen=False
   for s,l,w in select(q,r):
    ...
    try: c=cand(t,day,s,l,w,context,n,later); decision=engine.evaluate_candidate(c,mkt[day],{'planned_risk':0,'bucket_risk':{}},event_calendar=calendar)
     ...
     rows.append({'split':split,'ticker':t,'date':day,'status':'UNKNOWN','reason':type(exc).__name__,'selected':False,'option_rows':meta['option_rows_loaded']});continue
    opened=decision.action.value=='OPEN' and not chosen
    rows.append({'split':split,'ticker':t,'date':day,'status':'PASS' if opened else 'FAIL','reason':';'.join(decision.reason_codes) or decision.reason,'selected':opened,'option_rows':meta['option_rows_loaded'],'expiration':c.expiration,'short_strike':c.short_strike,'long_strike':c.long_strike,'credit':c.credit,'planned_loss':decision.planned_loss,'regime':decision.market_regime})
    if opened:
     chosen=True; expiry=pd.Timestamp(c.expiration); marks=load_spread_quotes_duckdb(':memory:',t,day,min(expiry,end),expiry,[c.short_strike,c.long_strike]); path=track_trade({'date':day,'expiration':expiry,'short_strike':c.short_strike,'long_strike':c.long_strike},marks,s,l,c.credit); ev=[z for z in path['events'].values() if z is not None]; exit_date=min(ev) if ev else (marks['Trade Date'].max() if not marks.empty else pd.NaT); blocked=expiry>end and (pd.isna(exit_date) or exit_date>=end)
