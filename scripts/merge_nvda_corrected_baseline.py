...
def atomic_frame(frame, path, csv=False):
    ...
    try:
        frame.to_csv(temp,index=False) if csv else frame.to_parquet(temp,index=False)
...
if len(all_candidates):
    ...
    atomic_frame(all_candidates,OUT/"candidates.parquet")
atomic_frame(annual,OUT/"annual_funnel.csv",csv=True)
funnel={"TRADING_DAYS":int(annual.trading_days.sum()),"FEATURE_READY_DAYS":int(annual.feature_ready_days.sum()),"SETUP_ELIGIBLE_DAYS":int(annual.setup_eligible_days.sum()),"CANDIDATE_SPREADS":int(annual.candidate_spreads.sum()),"SELECTED_ENTRIES":int(annual.selected_entries.sum()),"QUOTE_ROWS_ADAPTED":int(annual.quote_rows_adapted.sum()),"LIFECYCLES_COMPLETED":int(annual.lifecycles_completed.sum()),"EXECUTION_SEMANTICS":"quote_adaptation_only_no_lifecycle_exit_or_pnl"}
manifest={"research_id":"nvda_price_basis_corrected_authoritative_baseline_20260824","ticker":"NVDA","data_source":"PCS_CANONICAL_DATA","options_dataset":"options_v3","price_basis_version":"price_basis_v1","corporate_action_version":"authoritative_corporate_action_registry_v1","funnel":funnel,"annual":summaries,"final_oos_read":False,"production_rules_changed":False,"production_thresholds_changed":False,"lifecycle_fail_closed_reason":"CORPORATE_ACTION_CONTRACT_MAPPING_UNAVAILABLE","status":"QUOTE_ADAPTATION_ONLY","execution_semantics":"quote_adaptation_only_no_lifecycle_exit_or_pnl"}
atomic_json(manifest,OUT/"baseline_manifest.json")
...
atomic_json(split,OUT/"fresh_split_manifest.json")
