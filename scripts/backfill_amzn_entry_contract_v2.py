...
SOURCE=REPO_ROOT/"data/parquet/research/variant_b_full/AMZN_full_post2020_2d.parquet"
PHASE0=REPO_ROOT/"research_outputs/phase0_20260820/candidate_universe.parquet"
OUT=REPO_ROOT/"research_outputs/safe_strike_stage4a/authoritative_amzn_794_entry_contract_v2.parquet"
META=REPO_ROOT/"research_outputs/safe_strike_stage4a/authoritative_amzn_794_entry_contract_v2_readiness.json"
...
def main():
    ...
    access=PCSDataAccess(); daily=access.read_prices("AMZN", start_date=src.date.min(), end_date=src.date.max()); daily.date=pd.to_datetime(daily.date).dt.normalize(); confirmations={d:normalize_price_confirmation(analyze_entry_confirmation(daily,d).confirmation_score) for d in src.date.unique()}
    trend=pd.read_parquet(REPO_ROOT/"research_outputs/safe_strike_risk_map_v0_1/trend_histories/AMZN_trend.parquet"); trend.date=pd.to_datetime(trend.date).dt.normalize(); support=trend.drop_duplicates("date").set_index("date").support.map(lambda x: json.loads(x).get("nearest_support") if isinstance(x,str) else None)
    chain=access.read_quotes("AMZN", src.date.min(), src.date.max()); chain.trade_date=pd.to_datetime(chain.trade_date).dt.normalize(); chain.expiration_date=pd.to_datetime(chain.expiration_date).dt.normalize(); chains={d:g for d,g in chain.groupby("trade_date",sort=False)}
    close=daily.set_index("date").close; out=src.copy(); out["close"]=out.date.map(close); out["dte"]=(out.expiration-out.date).dt.days; out["expected_move_1d"]=out.apply(lambda r:calculate_expected_move(float(r.close),float(r.short_strike),float(r.atr),int(r.dte)).expected_move_1d,axis=1); out["support_level"]=out.date.map(support); out["option_volume"]=out.short_volume; out["open_interest"]=out.short_oi; out["bid_ask_pct"]=(out.short_ask-out.short_bid)/((out.short_ask+out.short_bid)/2).clip(lower=1e-12); out["nearby_strikes"]=out.apply(lambda r:nearby_strikes(chains.get(r.date,chain.iloc[0:0]),r.expiration,"p",r.short_strike),axis=1); out["later_expirations"]=out.apply(lambda r:later_expirations(chains.get(r.date,chain.iloc[0:0]),r.expiration,"p"),axis=1); out["price_confirmation"]=out.date.map(confirmations); out["entry_contract_version"]=ENTRY_CONTRACT_V2; out["contract_version"]=ENTRY_CONTRACT_V2; out["producer_version"]="backfill_amzn_entry_contract_v2:v1"; out["pit_asof_date"]=out.date; out["source_provenance"]="PURCHASED_VENDOR_BATCH_1:data/parquet/options_v2/symbol=AMZN"; out["canonical_option_source"]= "data/parquet/options_v2"
    ...
    try:
        meta_tmp.write_text(json.dumps(report,indent=2,default=str),encoding="utf-8")
