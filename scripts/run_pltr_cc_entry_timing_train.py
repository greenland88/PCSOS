"""Run implemented TRAIN entry-timing variants on the frozen PLTR population."""
import hashlib, json
...
runs = []
for timing in ("FIRST_MONTHLY", "UP_DAY", "POST_EARNINGS", "IV_RISING", "RESISTANCE_NEAR"):
    cfg = BaselineConfig(entry_timing=timing, profit_take_fraction=0.85,
                         roll_trigger="DTE_OR_ITM", prevalidate_paths=False)
    ...
    runs.append({"entry_timing": timing, "config": cfg.__dict__, "result": result})
    print(json.dumps({"timing": timing, "total": result["metrics"]["total_candidates"],
...
artifact = {"module": "pcs.covered_call_research.train_entry_timing_sweep", "version": "1.0",
...
(ROOT / "train_baseline" / "entry_timing_sweep_v1.json").write_text(json.dumps(artifact, indent=2, default=str), encoding="utf-8")
