...
def load_daily(symbol, end):
    ...
    x=pd.concat((pd.read_parquet(p) for p in ps),ignore_index=True).sort_values('date').drop_duplicates('date')
...
def main():
    ...
    # Each child owns an isolated in-memory DuckDB connection; option data is
    # still read exclusively from the persistent monthly Parquet store.
    result=run_backtest(stock,bench,option_root=f'data/parquet/options_monthly/{symbol}',start=args.start,end=args.end,backend='duckdb',duckdb_path=':memory:')
    rows=[]
