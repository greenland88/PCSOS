...
def daily(s,end):
 x=pd.concat((pd.read_parquet(p) for p in sorted((Path('data/parquet/daily')/f'symbol={s}').rglob('*.parquet'))),ignore_index=True).sort_values('date').drop_duplicates('date').reset_index(drop=True); x.date=pd.to_datetime(x.date); return x[x.date<=pd.Timestamp(end)].reset_index(drop=True)
def eligible(s,a,b):
...
def metric(x,s):
 d=daily(s,W[s][1]).set_index('date'); touch={}
 for h in (5,10): touch[h]=np.mean([((d.loc[pd.Timestamp(r.date)+pd.tseries.offsets.BDay(1):pd.Timestamp(r.date)+pd.tseries.offsets.BDay(h),'low']<=r.short_strike).any()) for _,r in x.iterrows()]) if len(x) else np.nan
