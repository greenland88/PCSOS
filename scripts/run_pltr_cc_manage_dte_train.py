"""Run TRAIN-only management-DTE sweep on the frozen PLTR population."""
import hashlib
import json
...
runs = []
for manage_dte in (21, 14, 7, 3):
    cfg = BaselineConfig(manage_dte=manage_dte, profit_take_fraction=0.85,
    ...
    runs.append({"manage_dte": manage_dte, "config": cfg.__dict__, "result": result})
    print(json.dumps({"manage_dte": manage_dte, "total": result["metrics"]["total_candidates"],
...
artifact = {"module": "pcs.covered_call_research.train_manage_dte_sweep",
...
(ROOT / "train_baseline" / "manage_dte_sweep_v1.json").write_text(json.dumps(artifact, indent=2, default=str), encoding="utf-8")
