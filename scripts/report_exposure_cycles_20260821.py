...
def one(ticker, split):
 ...
 for i,group in enumerate(cycles,1):
  ...
  if pd.notna(finish): days=len(pd.bdate_range(start,finish))
  else: days=None
 ...
 if len(out)>1:
  ...
  out.loc[out.index[:-1],'gap_to_next_cycle']=[max(0,len(pd.bdate_range(a+pd.Timedelta(days=1),b-pd.Timedelta(days=1)))) for a,b in zip(out.cycle_end.iloc[:-1],out.next_cycle_start.iloc[:-1])]
 return out
...
def main():
 ...
 for t in ('SPY','QQQ'):
  for s in ('TRAIN','VALIDATION'):
   d=one(t,s); d.to_csv(OUT/f'{t.lower()}_{s.lower()}_cycles.csv',index=False); allrows.append(d)
