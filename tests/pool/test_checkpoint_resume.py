...
def test_saved_timing_continues_without_recomputing_indicators(monkeypatch):
    ...
    assert result.candidate_state["trend_evidence"] == saved[0].stage_state["evidence"]["trend_evidence"]
    assert result.candidate_state["price_indicator_series"] == saved[0].stage_state["evidence"]["price_indicator_series"]
