...
def main():
 ...
 selected=tuple(s for s in req['symbols'] if records[s]['final']['status']=='READY')
 ...
 request=ReadOnlyScanRequest(symbols=selected,universe_id='frozen_2953_verified_daily_subset',
  ...
  output_directory=str(out/'read_only_scan'))
 from dataclasses import asdict
 (out/'scan_request.json').write_text(json.dumps({'request':asdict(request),'parent_universe_count':2953,
 ...
 payload=result.to_dict();(out/'scan_result.json').write_text(json.dumps(payload,indent=2,default=str))
 after=hashlib.sha256(manifest.read_bytes()).hexdigest();assert before==after,'READ_ONLY_MANIFEST_CHANGED'
 (out/'scan_read_only_proof.json').write_text(json.dumps({'before':before,'after':after,'canonical_manifest_unchanged':True,
