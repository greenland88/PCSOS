...
| Verifiable support zones | IMPLEMENTED — REAL SAMPLE ACCEPTANCE PENDING | `pcs.trend.support_zones.evaluate_support_zones(SupportZoneInput)` builds fixed, bounded SMA/confirmed-swing zones, causal support tests, frozen invalidation lines, replayable state and per-day history. Research/description schema 1.0; old SupportResult and trading behavior unchanged. Read-only CLI: `support-zones`; contract and validation: `docs/operations/selection_v2/step_03.md`. |
