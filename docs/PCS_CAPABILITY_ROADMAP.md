...
| Stock observation and discussion packets | IN_PROGRESS — STEP 8 REVIEW | Explicit `stock-shortlist`, independent typed ranking/packet/query/AI-review import APIs. Saved derived inputs only; no detector, canonical, provider, model or production scan calls. See [Step 8 handoff](operations/selection_v2/step_08.md). |
