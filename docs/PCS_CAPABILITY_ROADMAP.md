...
| Entry opportunity state / healthy pullback | IMPLEMENTED — STEP 4 OBSERVATION; REAL COVERAGE 7/8 | `pcs.trend.opportunity_engine.evaluate_entry_opportunity(OpportunityInput)` provides the shared causal NO_SETUP/WATCH/CONFIRMING/ENTRY_READY/EXPIRED/INVALIDATED lifecycle, fixed support binding, six confirmation gates, frozen confirmation/entry dates, capability gaps, stable recovery and evidence views. Schema 1.0 / entry-opportunity-v2 / healthy-pullback-opportunity-v1.7. 84 focused checks; clean canonical acceptance at 2026-09-04 produced 60-day timelines for 7 symbols; UBER retained INSUFFICIENT_FEATURE_WARMUP. Old replay, pool admission, options and final action remain unchanged. CLI: `entry-opportunity`; evidence: `docs/operations/selection_v2/step_04.md`. |
